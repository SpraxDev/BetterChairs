package betterchairs.nms.v1_21_R6;
...
class CustomArmorStand extends ArmorStand implements CustomChairEntity {
    ...
    public void tick() {
        ...
        if (ChairUtils.didChairEntityMove(this.expectedLocation, getX(), getY(), getZ())) {
            ...
            teleportTo(this.expectedLocation.getX(), this.expectedLocation.getY(), this.expectedLocation.getZ());
        }
    ...
    public void kill(ServerLevel serverLevel) {
        ...
        if (shouldDie()) super.kill(serverLevel);
    }

    @Override
    public void kill(ServerLevel serverLevel, DamageSource damageSource) {
        // Prevents the ArmorStand from getting killed unexpectedly
        if (shouldDie()) super.kill(serverLevel, damageSource);
    }
